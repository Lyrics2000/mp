...
from config.util.mpesautils import get_token
...
class AuthTokenManager(models.Manager):
    ...
    def get_token(self, type,client_ref,client_secret,development):
        current_time = int(time.time())
        if self.filter(type=type.lower()).count() == 0:
            # means we don't have a record yet so we create
            token = get_token(type.lower(),client_ref,client_secret,development)
