...
# echo yes | python manage.py migrate --database=business_schema

# DJANGO_PROJECT_DIR="python"
...
exec gunicorn --bind=0.0.0.0:8075 config.wsgi --workers=5 --log-level=info --access-logfile=- --error-logfile=- --timeout=30000
# exec gunicorn --bind=0.0.0.0:9091 config.wsgi --workers=5 --log-level=info --access-logfile=- --error-logfile=- --timeout=30000 --reload
